...
<nav class="h-14 w-screen fixed flex font-hubotSansXbold">
  <div class="w-full flex items-center justify-center">
    <a href="/"><svg class="fill-white stroke-white hover:fill-purple-300 hover:stroke-purple-300" width="65" height="46" viewBox="0 0 65 46" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  {#if open}
   <ul in:fly={{x:100, duration:500}} out:fly={{x:100, duration:500}} class="bg-transparent backdrop-blur-xl h-screen w-screen absolute flex flex-col items-center justify-evenly text-white text-2xl right-0 md:w-1/3">
    <li class="border-b-2 border-transparent p-3" class:active={$page.url.pathname === '/'}><a on:click={onClick} on:keyup={onClick} href="/">HOME</a></li>
    <li class="border-b-2 border-transparent p-3" class:active={$page.url.pathname === '/about'}><a on:click={onClick} on:keyup={onClick} href="/about">ABOUT ME</a></li>
    <li class="border-b-2 border-transparent p-3" class:active={$page.url.pathname === '/projects'}><a on:click={onClick} on:keyup={onClick} href="/projects">PROJECTS</a></li>
    <li class="border-b-2 border-transparent p-3" class:active={$page.url.pathname === '/contact'}><a on:click={onClick} on:keyup={onClick} href="contact">CONTACT ME</a></li>
   </ul>
  {/if}
</nav>
